...
-- ════════════════════════════════════════════════════════════
--  🔔 THEME REGISTRY — ทุก element ลงทะเบียน recolor fn ไว้
-- ════════════════════════════════════════════════════════════
local _themeListeners = {}

-- ลงทะเบียน callback รับ theme ใหม่เมื่อเปลี่ยน
local function onThemeChange(fn)
    table.insert(_themeListeners, fn)
end

-- เรียก callback ทั้งหมด (ใน pcall กัน error ถ้า element destroy ไปแล้ว)
...
-- ════════════════════════════════════════════════════════════
--  🔵 SYSTEM 0 — Page Transition (fade ทั้งหน้าต่าง)
-- ════════════════════════════════════════════════════════════

-- ใช้ overlay ระดับ Win แทน page-level เพื่อ fade ทุกอย่าง
local _winOverlay = nil  -- สร้างครั้งเดียว ใช้ซ้ำ

local function getWinOverlay(win)
    if _winOverlay and _winOverlay.Parent then return _winOverlay end
    -- parent คือ ScreenGui (ui) เพื่อหลีกเลี่ยง ClipsDescendants ของ Win
    local ui = win and win.Parent
    local ov = Instance.new("Frame")
    ov.Name                   = "__WinFadeOverlay"
    ov.Parent                 = ui or win
    ov.BackgroundColor3       = TH.bg
    ov.BorderSizePixel        = 0
    -- ขนาด+ตำแหน่งตาม Win
    ov.Position               = win and win.Position or UDim2.new(0,0,0,0)
    ov.Size                   = win and win.Size     or UDim2.new(1,0,1,0)
    ov.ZIndex                 = 9999
    ov.BackgroundTransparency = 1
    ov.Active                 = false
    -- ติดตามขนาด/ตำแหน่ง Win
    if win then
        win:GetPropertyChangedSignal("Position"):Connect(function() ov.Position = win.Position end)
        win:GetPropertyChangedSignal("Size"):Connect(function()     ov.Size     = win.Size     end)
    end
    local _ovc = Instance.new("UICorner"); _ovc.CornerRadius = UDim.new(0,6); _ovc.Parent = ov
    _winOverlay = ov
    return ov
end

local function pageOut(pageFrame, done, win)
    local ov = getWinOverlay(win or (pageFrame and pageFrame:FindFirstAncestorOfClass("Frame")))
    if not ov then if done then done() end return end
    ov.BackgroundColor3 = TH.bg
    ov.BackgroundTransparency = 1
    ov.Active = true
    tw(ov, {BackgroundTransparency = 0}, 0.12, Enum.EasingStyle.Sine)
    task.delay(0.13, function()
        if done then done() end
    end)
end

local function pageIn(pageFrame, win)
    local ov = _winOverlay
    if not ov or not ov.Parent then return end
    task.delay(0.04, function()
        tw(ov, {BackgroundTransparency = 1}, 0.20, Enum.EasingStyle.Sine)
        task.delay(0.21, function()
            if ov and ov.Parent then ov.Active = false end
        end)
    end)
end
...
function Library:Window(windowTitle, SizeUI)
    ...
    -- Win
    -- ════════════════════════════════════════════════════════════
    --  📐 ขนาดหน้าต่างเป้าหมาย (เซฟไว้ก่อน สร้าง Win ทีหลัง)
    -- ════════════════════════════════════════════════════════════
    local targetW = SizeUI and SizeUI.X.Offset or 560
    local targetH = SizeUI and SizeUI.Y.Offset or 340
    -- Win position กึ่งกลางจอ
    local targetPosX = -math.floor(targetW / 2)
    local targetPosY = -math.floor(targetH / 2)
    ...
    Win.Position         = UDim2.new(0.5, targetPosX, 0.5, targetPosY)
    Win.Size             = SizeUI or UDim2.new(0, targetW, 0, targetH)
    Win.Active           = true
    Win.Draggable        = true
    Win.ClipsDescendants = true
    -- เริ่มต้นซ่อน เอา open animation ทำงานก่อน
    Win.Visible          = false
    ...
    do
        ...
        ResizeBtn.MouseButton1Down:Connect(function()
            lx = UserInputService:GetMouseLocation().X
            ly = UserInputService:GetMouseLocation().Y
            local mc,rc
            mc = mouse.Move:Connect(function()
                local dx = UserInputService:GetMouseLocation().X - lx
                local dy = UserInputService:GetMouseLocation().Y - ly
                Win.Size = Win.Size + UDim2.new(0,dx,0,dy)
                lx = UserInputService:GetMouseLocation().X
                ly = UserInputService:GetMouseLocation().Y
            end)
            rc = UserInputService.InputEnded:Connect(function(inp)
        ...
        Win:GetPropertyChangedSignal("Size"):Connect(function()
            local s = Win.Size
            if s.X.Offset < minW then Win.Size = UDim2.new(0,minW,0,s.Y.Offset) end
            s = Win.Size
            if s.Y.Offset < minH then Win.Size = UDim2.new(0,s.X.Offset,0,minH) end
        end)
    end

    -- ลงทะเบียน Window-level elements เข้า theme registry
    ...
    -- ════════════════════════════════════════════════════════════
    --  🎬 OPEN / COLLAPSE ANIMATION SYSTEM
    --  RCtrl → collapse/expand
    --  เปิดครั้งแรก: pill ลอยขึ้นจากล่างจอ → ขยายเป็น window
    -- ════════════════════════════════════════════════════════════

    local _isCollapsed   = false
    local _animBusy      = false
    local _savedWinPos   = Win.Position  -- จำตำแหน่งจริง

    -- ── อุปกรณ์ประกอบฉาก: Mini Pill ──────────────────────────
    ...
    Pill.Position         = UDim2.new(0.5, 0, 1, 60)   -- เริ่มต้นซ่อนใต้จอ
    Pill.Size             = UDim2.new(0, 180, 0, 36)
    Pill.ZIndex           = 5
    Pill.Visible          = false
    local _pillC = Instance.new("UICorner"); _pillC.CornerRadius = UDim.new(0,18); _pillC.Parent = Pill
    local PillStroke = Instance.new("UIStroke")
    PillStroke.Parent = Pill; PillStroke.Color = TH.br; PillStroke.Thickness = 1.5

    -- dot สี accent
    local PillDot = Instance.new("Frame")
    PillDot.Parent = Pill; PillDot.AnchorPoint = Vector2.new(0,0.5)
    PillDot.BackgroundColor3 = TH.ac; PillDot.BorderSizePixel = 0
    PillDot.Position = UDim2.new(0,12,0.5,0); PillDot.Size = UDim2.new(0,8,0,8)
    local _dotC = Instance.new("UICorner"); _dotC.CornerRadius = UDim.new(1,0); _dotC.Parent = PillDot

    -- ชื่อ
    local PillLbl = Instance.new("TextLabel")
    PillLbl.Parent = Pill; PillLbl.BackgroundTransparency = 1
    PillLbl.Position = UDim2.new(0,26,0,0); PillLbl.Size = UDim2.new(1,-50,1,0)
    PillLbl.Font = Enum.Font.GothamBold; PillLbl.Text = windowTitle or "Arsenal Hub"
    PillLbl.TextColor3 = WT; PillLbl.TextSize = 11
    PillLbl.TextXAlignment = Enum.TextXAlignment.Left

    -- hint key
    local PillKey = Instance.new("TextLabel")
    PillKey.Parent = Pill; PillKey.BackgroundTransparency = 1
    PillKey.AnchorPoint = Vector2.new(1,0.5)
    PillKey.Position = UDim2.new(1,-10,0.5,0); PillKey.Size = UDim2.new(0,30,0,20)
    PillKey.Font = Enum.Font.GothamBold; PillKey.Text = "▲"
    PillKey.TextColor3 = TH.su; PillKey.TextSize = 10

    -- pill click → expand
    local PillBtn = Instance.new("TextButton")
    PillBtn.Parent = Pill; PillBtn.BackgroundTransparency = 1
    PillBtn.Size = UDim2.new(1,0,1,0); PillBtn.Text = ""; PillBtn.ZIndex = 6

    onThemeChange(function(t)
        tw(Pill, {BackgroundColor3 = t.bg}, 0.22)
        PillStroke.Color = t.br
        PillDot.BackgroundColor3 = t.ac
        PillKey.TextColor3 = t.su
    end)

    -- ── helper: ซ่อน/แสดง content ทั้งหมดใน Win ─────────────
    local _contentParts = { TBar, Sidebar, ContentArea }

    local function setContentVisible(visible, fade)
        local tr = visible and 0 or 1
        local dur = fade or 0.18
        for _, f in ipairs(_contentParts) do
            if visible then
                f.Visible = true
                tw(f, {BackgroundTransparency = 0}, dur, Enum.EasingStyle.Sine)
            else
                tw(f, {BackgroundTransparency = 1}, dur, Enum.EasingStyle.Sine)
                task.delay(dur + 0.02, function()
                    if not visible then f.Visible = false end
                end)
            end
        end
    end

    -- ── OPEN ANIMATION (เปิดครั้งแรก) ────────────────────────
    -- 1) Pill โผล่จากล่าง
    -- 2) ขยาย Pill → ขนาด Win (background โล่ง)
    -- 3) เปิดเนื้อหาทีละส่วน
    local function playOpenAnimation()
        if _animBusy then return end
        _animBusy = true
        ...
        -- step 1: เลื่อน pill ขึ้นมาใกล้ก้นจอ (0.30s)
        tw(Pill, {Position = UDim2.new(0.5, 0, 1, -20)}, 0.30, Enum.EasingStyle.Back, Enum.EasingDirection.Out)

        task.delay(0.38, function()
            -- step 2: ขยาย pill → ขนาด win (background ใส เห็นแค่ outline)
            -- คำนวณ position กึ่งกลางจอ
            local cx = 0.5; local cxO = targetPosX
            local cy = 0.5; local cyO = targetPosY

            -- เปลี่ยน pill anchor ไป 0.5,0.5 ก่อน
            Pill.AnchorPoint = Vector2.new(0.5, 0.5)
            Pill.Position    = UDim2.new(0.5, 0, 1, -(18 + 20)) -- กึ่งกลาง pill เดิม

            -- ขยายไปเต็ม target size (0.45s, Quart)
            tw(Pill, {
                Position = UDim2.new(cx, cxO, cy, cyO),
                Size     = UDim2.new(0, targetW, 0, targetH),
            }, 0.45, Enum.EasingStyle.Quart, Enum.EasingDirection.Out)

            task.delay(0.50, function()
                -- step 3: แสดง Win (พื้นหลังเต็ม) แล้วซ่อน Pill
                Win.Position = UDim2.new(cx, cxO, cy, cyO)
                ...
                -- step 4: fade in content ทีละส่วน
                task.delay(0.06, function()
                    TBar.Visible = true
                    tw(TBar, {BackgroundTransparency = 0}, 0.22, Enum.EasingStyle.Sine)
                end)
                ...
                task.delay(0.28, function()
                    ...
                    tw(ContentArea, {BackgroundTransparency = 0}, 0.25, Enum.EasingStyle.Sine)
                    task.delay(0.28, function()
                        _animBusy = false
                        _savedWinPos = Win.Position
                    end)
                end)
            end)
        end)
    ...
    local function collapseWindow()
        ...
        for _, f in ipairs(_contentParts) do
            tw(f, {BackgroundTransparency = 1}, 0.15, Enum.EasingStyle.Sine)
        end
        tw(Win, {BackgroundTransparency = 1}, 0.15, Enum.EasingStyle.Sine)
        WinStroke.Transparency = 1

        task.delay(0.18, function()
            Win.Visible = false
            for _, f in ipairs(_contentParts) do f.Visible = false end

            -- แสดง pill ที่ตำแหน่งปัจจุบันของ win (กึ่งกลาง)
            local winCX = Win.AbsolutePosition.X + Win.AbsoluteSize.X * 0.5
            local winCY = Win.AbsolutePosition.Y + Win.AbsoluteSize.Y * 0.5
            local vpX   = workspace.CurrentCamera.ViewportSize.X
            local vpY   = workspace.CurrentCamera.ViewportSize.Y

            Pill.AnchorPoint = Vector2.new(0.5, 0.5)
            Pill.Position    = UDim2.new(0, winCX, 0, winCY)
            Pill.Size        = UDim2.new(0, targetW, 0, targetH)
            ...
            tw(PillStroke, {Transparency = 0}, 0.35, Enum.EasingStyle.Sine)

            -- เลื่อนลงก้นจอ
            tw(Pill, {Position = UDim2.new(0.5, 0, 1, -20)}, 0.40, Enum.EasingStyle.Quart, Enum.EasingDirection.Out)
    ...
    local function expandWindow()
        ...
        -- ขยาย pill → ขนาด win
        local cx  = 0.5
        local cxO = _savedWinPos.X.Offset
        local cyO = _savedWinPos.Y.Offset

        tw(Pill, {
            Position = UDim2.new(cx, cxO, 0.5, cyO),
            Size     = UDim2.new(0, targetW, 0, targetH),
        }, 0.40, Enum.EasingStyle.Quart, Enum.EasingDirection.Out)

        task.delay(0.44, function()
            -- แสดง Win โล่ง
            Win.Position = _savedWinPos
            Win.Size     = UDim2.new(0, targetW, 0, targetH)
            Win.BackgroundTransparency = 0
            WinStroke.Transparency = 0
            Win.Visible  = true
            ...
            task.delay(0.15, function()
                Sidebar.Visible = true
                tw(Sidebar, {BackgroundTransparency = 0}, 0.20, Enum.EasingStyle.Sine)
            end)
            task.delay(0.24, function()
                ContentArea.Visible = true
                tw(ContentArea, {BackgroundTransparency = 0}, 0.22, Enum.EasingStyle.Sine)
    ...
    PillBtn.MouseButton1Click:Connect(function()
        if not _isCollapsed or _animBusy then return end
        expandWindow()
    end)
    ...
    -- ── เรียก open animation หลัง frame แรก ────────────────
    task.defer(function()
        task.wait(0.05)
        playOpenAnimation()
    end)
    ...
    local function calcIndicPos(idx)
        local h2     = math.floor(BTN_H * 0.55)
        local btnTop = PAD_T + (idx-1)*(BTN_H+BTN_G)
        local top    = btnTop + math.floor((BTN_H-h2)/2)
        local tlfOffY = TabListFrame.Position.Y.Offset
        local scrollY = TabListFrame.CanvasPosition.Y
        return tlfOffY + top - scrollY, h2
    end

    local function moveIndic(idx)
        if idx == 0 then return end
        local top, h2 = calcIndicPos(idx)
        tw(Indic, {Size = UDim2.new(0,3,0,h2)}, 0.12, Enum.EasingStyle.Quad)
        tw(Indic, {Position = UDim2.new(0,0,0,top)}, 0.35, Enum.EasingStyle.Quint)
        task.delay(0.12, function()
            tw(Indic, {Size = UDim2.new(0,3,0,h2)}, 0.22, Enum.EasingStyle.Back)
        end)
    end

    TabListFrame:GetPropertyChangedSignal("CanvasPosition"):Connect(function()
        for i,t in ipairs(allTabBtns) do
            if t.isActive and t.isActive() then
                local top, h2 = calcIndicPos(i)
    ...
    -- init overlay บน Win ทันที
    getWinOverlay(Win)

    -- อัปเดตสี overlay เมื่อเปลี่ยน theme
    onThemeChange(function(t)
        if _winOverlay then _winOverlay.BackgroundColor3 = t.bg end
    end)

    -- switchToPane ด้วย overlay fade ครอบทั้งหน้าต่าง
    local function switchToPane(newIdx, newPage)
        if newIdx == currentIdx then return end
        if transitioning then
            UIPageLayout:JumpToIndex(newIdx - 1)
            currentIdx  = newIdx
            currentPage = newPage
            return
        end
    ...
    function Tabs:Tab(tabName, tabIcon)
        ...
        TabBtn.Text                   = ""
        TabBtn.ZIndex                 = 2
        local _tbc = Instance.new("UICorner"); _tbc.CornerRadius = UDim.new(0,7); _tbc.Parent = TabBtn
        ...
        TIcon.Size                 = UDim2.new(1,0,0,20)
        TIcon.Font                 = Enum.Font.GothamBold
        TIcon.Text                 = tabIcon or "•"
        TIcon.TextColor3           = TH.su
        TIcon.TextSize             = 17
        TIcon.ZIndex               = 3
        ...
        TabBtn.MouseEnter:Connect(function()
            if not isActive then
                tw(TabBtn, {BackgroundTransparency = 0.92}, 0.14, Enum.EasingStyle.Sine)
            end
        end)
        TabBtn.MouseLeave:Connect(function()
            if not isActive then
                tw(TabBtn, {BackgroundTransparency = 1}, 0.14, Enum.EasingStyle.Sine)
            end
        end)
        ...
        -- ลงทะเบียน page ใน theme registry
        onThemeChange(function(t)
            Page.BackgroundColor3    = t.pn
            Page.ScrollBarImageColor3 = t.ac
        end)
        ...
        table.insert(allTabBtns, {
            ...
            icon      = TIcon,
            lbl       = TLbl,
            setActive = function(v) isActive = v end,
            isActive  = function() return isActive end,
        })

        -- ลงทะเบียน tab button ใน theme registry
        onThemeChange(function(t)
            if not isActive then
                tw(TIcon, {TextColor3 = t.su}, 0.18)
                tw(TLbl,  {TextColor3 = t.su}, 0.18)
            else
                tw(TIcon, {TextColor3 = WT}, 0.18)
                tw(TLbl,  {TextColor3 = WT}, 0.18)
                tw(TabBtn, {BackgroundColor3 = t.dk}, 0.18)
            end
        end)
        ...
        local function activate()
            for _, t in ipairs(allTabBtns) do
                ...
                tw(t.icon, {TextColor3 = TH.su}, 0.15)
                tw(t.lbl,  {TextColor3 = TH.su}, 0.15)
            end
            isActive = true
            TabBtn.BackgroundColor3       = TH.dk
            TabBtn.BackgroundTransparency = 0
            tw(TIcon, {TextColor3 = WT}, 0.15)
        ...
        function TabFunctions:Toggle(label, on, callback)
            ...
            Track.MouseButton1Click:Connect(function()
                toggled = not toggled
                tw(Track, {Size = UDim2.new(0,40,0,20)}, 0.12, Enum.EasingStyle.Back)
                tw(Track, {BackgroundColor3 = toggled and TH.ac or TH.sl}, 0.17)
                tw(Knob,  {Position = toggled and UDim2.new(0,22,0.5,0) or UDim2.new(0,2,0.5,0)}, 0.17)
                callback(toggled)
            end)

            onThemeChange(function(t)
                tw(Track, {BackgroundColor3 = toggled and t.ac or t.sl}, 0.22)
            end)
        end
        ...
        function TabFunctions:Slider(label, mn, mx, val, callback)
            ...
            ResetBtn.Position             = UDim2.new(1,0,0,2)
            ResetBtn.Size                 = UDim2.new(0,20,0,20)
            ResetBtn.Font                 = Enum.Font.GothamBold
            ResetBtn.Text                 = "↺"
            ResetBtn.TextColor3           = TH.su
            ResetBtn.TextSize             = 13
            ResetBtn.AutoButtonColor      = false
            ResetBtn.ZIndex               = 3

            ResetBtn.MouseEnter:Connect(function() tw(ResetBtn,{TextColor3=TH.ac},0.12) end)
            ResetBtn.MouseLeave:Connect(function() tw(ResetBtn,{TextColor3=TH.su},0.12) end)

            -- TextBox ตัวเลข
            local SLVal = Instance.new("TextBox")
            SLVal.Parent               = Wrap
            SLVal.AnchorPoint          = Vector2.new(1,0)
            SLVal.BackgroundTransparency = 1
            SLVal.BorderSizePixel      = 0
            SLVal.Position             = UDim2.new(1,-22,0,2)
            SLVal.Size                 = UDim2.new(0,46,0,20)
            ...
            SLTrack.Parent           = Wrap
            SLTrack.AnchorPoint      = Vector2.new(0,1)
            SLTrack.BackgroundColor3 = TH.sl
            SLTrack.BorderSizePixel  = 0
            SLTrack.Position         = UDim2.new(0,2,1,-4)
            SLTrack.Size             = UDim2.new(1,-4,0,5)
            ...
            SLKnob.ZIndex           = 2
            local _skc = Instance.new("UICorner"); _skc.CornerRadius = UDim.new(1,0); _skc.Parent = SLKnob
            ...
            SLTrack.MouseEnter:Connect(function()
                tw(SLKnob, {Size = UDim2.new(0,17,0,17)}, 0.12, Enum.EasingStyle.Sine)
            end)
            SLTrack.MouseLeave:Connect(function()
                if not dragging then tw(SLKnob, {Size = UDim2.new(0,14,0,14)}, 0.12, Enum.EasingStyle.Sine) end
            end)
            ...
            SLTrack.InputBegan:Connect(function(inp)
                if inp.UserInputType == Enum.UserInputType.MouseButton1 then
                    dragging = true
                    tw(SLKnob, {Size = UDim2.new(0,17,0,17)}, 0.1)
                    doSlide(inp.Position.X)
                end
            end)
            SLTrack.InputEnded:Connect(function(inp)
                if inp.UserInputType == Enum.UserInputType.MouseButton1 then
                    dragging = false
                    tw(SLKnob, {Size = UDim2.new(0,14,0,14)}, 0.12, Enum.EasingStyle.Sine)
                end
            end)
            ...
            -- พิมพ์ตัวเลขได้ กรองเฉพาะตัวเลข
            ...
            onThemeChange(function(t)
                ...
                tw(ResetBtn,   {TextColor3 = t.su}, 0.22)
        ...
        function TabFunctions:Button(label, callback)
            ...
            Btn.MouseEnter:Connect(function()
                hovered = true
                tw(Btn, {BackgroundColor3 = TH.ac, TextColor3 = Color3.new(1,1,1)}, 0.14)
            end)
            Btn.MouseLeave:Connect(function()
                hovered = false
                tw(Btn, {BackgroundColor3 = TH.dk, TextColor3 = TH.ac}, 0.14)
            end)
            Btn.MouseButton1Down:Connect(function()
                tw(Btn, {Size = UDim2.new(1,-32,0,26)}, 0.08, Enum.EasingStyle.Sine)
            end)
            Btn.MouseButton1Click:Connect(function()
                tw(Btn, {Size = UDim2.new(1,-28,0,28)}, 0.14, Enum.EasingStyle.Back)
                callback()
            end)

            onThemeChange(function(t)
                if not hovered then
                    tw(Btn, {BackgroundColor3 = t.dk, TextColor3 = t.ac}, 0.22)
                end
            end)
        end
        ...
        function TabFunctions:Single(label, opts, default, callback)
            ...
            for _, opt in ipairs(opts) do
                ...
                table.insert(optEls, {btn=Item, radio=Radio, radioStroke=RadioStroke, radioDot=RadioDot, otx=OTx, xbtn=XBtn})

                Item.MouseEnter:Connect(function()
                    if opt ~= curSel then tw(Item,{BackgroundTransparency=0.94},0.12) end
                end)
                Item.MouseLeave:Connect(function()
                    if opt ~= curSel then tw(Item,{BackgroundTransparency=1},0.12) end
                end)
                Item.MouseButton1Click:Connect(function()
                    clearAll()
                    curSel = opt
                    tw(Radio, {BackgroundColor3 = TH.ac}, 0.13)
                    RadioStroke.Color        = TH.ac
                    RadioDot.TextTransparency = 0
                    tw(OTx,  {TextColor3 = WT}, 0.12)
            ...
            onThemeChange(function(t)
                ...
                for _, el in ipairs(optEls) do
                    if el.btn.BackgroundTransparency < 0.5 then -- selected
        ...
        function TabFunctions:ColorPicker(label, initColor, callback)
            ...
            HexBox.Text                 = "#"..color3ToHex(curColor)
            ...
            RgbBox.Text                 = color3ToRgbString(curColor)
            ...
            -- sync boxes
            local origUpdate = updateAll
            updateAll = function()
                origUpdate()
                if not HexBox:IsFocused() then HexBox.Text = "#"..color3ToHex(curColor) end
                if not RgbBox:IsFocused() then RgbBox.Text = color3ToRgbString(curColor) end
            end
        ...
        -- ══════════════════════════════════════════════════
        --  🎨 SYSTEM 2 — Theme Selector (preset only)
        -- ══════════════════════════════════════════════════
        function TabFunctions:ThemeSelector()
            ...
            EditBtn.Position = UDim2.new(1,0,0.5,0); EditBtn.Size = UDim2.new(0,44,0,16)
            ...
            NameBox.Parent = NameWrap; NameBox.BackgroundColor3 = TH.cd
            NameBox.BorderSizePixel = 0; NameBox.Size = UDim2.new(1,0,1,0)
            NameBox.Font = Enum.Font.Gotham; NameBox.Text = ""
            NameBox.TextColor3 = WT; NameBox.TextSize = 11
            NameBox.PlaceholderText = "ชื่อธีม..."; NameBox.ClearTextOnFocus = false
            local _nbc = Instance.new("UICorner"); _nbc.CornerRadius = UDim.new(0,7); _nbc.Parent = NameBox
            local NameStroke = Instance.new("UIStroke"); NameStroke.Parent=NameBox; NameStroke.Color=TH.br; NameStroke.Thickness=1
            onThemeChange(function(t)
                tw(NameBox,{BackgroundColor3=t.cd},0.22); NameStroke.Color=t.br
            end)
            ...
            SaveBtn.Parent = Page; SaveBtn.BackgroundColor3 = TH.dk
            SaveBtn.BorderSizePixel = 0; SaveBtn.Size = UDim2.new(1,-28,0,30)
            SaveBtn.AutoButtonColor = false; SaveBtn.Font = Enum.Font.GothamBold
            SaveBtn.Text = "＋  บันทึกธีมนี้"; SaveBtn.TextColor3 = TH.ac; SaveBtn.TextSize = 12
            local _svbc = Instance.new("UICorner"); _svbc.CornerRadius = UDim.new(0,7); _svbc.Parent = SaveBtn

            local saveHov = false
            SaveBtn.MouseEnter:Connect(function() saveHov=true; tw(SaveBtn,{BackgroundColor3=TH.ac,TextColor3=Color3.new(1,1,1)},0.14) end)
            SaveBtn.MouseLeave:Connect(function() saveHov=false; tw(SaveBtn,{BackgroundColor3=TH.dk,TextColor3=TH.ac},0.14) end)
            SaveBtn.MouseButton1Down:Connect(function() tw(SaveBtn,{Size=UDim2.new(1,-32,0,28)},0.08,Enum.EasingStyle.Sine) end)
            SaveBtn.MouseButton1Click:Connect(function()
                tw(SaveBtn,{Size=UDim2.new(1,-28,0,30)},0.14,Enum.EasingStyle.Back)
                local snap = {}
                for k,v in pairs(editTheme) do snap[k]=v end
            ...
            onThemeChange(function(t)
                if saveHov then tw(SaveBtn,{BackgroundColor3=t.ac,TextColor3=Color3.new(1,1,1)},0.22)
                else tw(SaveBtn,{BackgroundColor3=t.dk,TextColor3=t.ac},0.22) end
            end)

            -- ── Export / Import ───────────────────────────────────────
            TabFunctions.Section(self, "แชร์โค้ดธีม")

            -- encode: 9 สี × 6 hex = 54 ตัวอักษร
            local function encodeTheme(t)
                local out = t.name and (t.name:gsub("[^%w%-_]","_"):sub(1,16)..":") or ""
                for _, k in ipairs(SLOT_KEYS) do
                    out = out .. color3ToHex(t[k])
                end
                return out
            end

            local function decodeTheme(code, fallbackName)
                -- strip whitespace
                code = code:gsub("%s","")
                -- optional name prefix: "Name:HEXHEX..."
                local name, hex = code:match("^([^:]+):([%x]+)$")
                if not name then
                    name = fallbackName or "Imported"
                    hex  = code:gsub("[^%x]",""):upper()
                else
                    hex = hex:upper()
                end
                if #hex ~= 54 then return nil end
                local t = { name = name }
                for i, k in ipairs(SLOT_KEYS) do
                    local s = (i-1)*6 + 1
                    local c = hexToColor3(hex:sub(s,s+5))
            ...
            -- ── Export row ──────────────────────────────────────
            local ExportLbl = Instance.new("TextLabel")
            ExportLbl.Parent = Page; ExportLbl.BackgroundTransparency = 1
            ExportLbl.Size = UDim2.new(1,-28,0,14); ExportLbl.Font = Enum.Font.Gotham
            ExportLbl.Text = "โค้ดสำหรับแชร์ — copy แล้วส่งให้เพื่อน"
            ExportLbl.TextColor3 = TH.su; ExportLbl.TextSize = 9
            ExportLbl.TextXAlignment = Enum.TextXAlignment.Left
            onThemeChange(function(t) tw(ExportLbl,{TextColor3=t.su},0.22) end)

            local ExportWrap = Instance.new("Frame")
            ExportWrap.Parent = Page; ExportWrap.BackgroundTransparency = 1
            ExportWrap.BorderSizePixel = 0; ExportWrap.Size = UDim2.new(1,-28,0,30)
            local ExportLayout = Instance.new("UIListLayout")
            ExportLayout.Parent = ExportWrap; ExportLayout.FillDirection = Enum.FillDirection.Horizontal
            ExportLayout.SortOrder = Enum.SortOrder.LayoutOrder; ExportLayout.Padding = UDim.new(0,6)
            ExportLayout.VerticalAlignment = Enum.VerticalAlignment.Center

            local ExportBox = Instance.new("TextBox")
            ExportBox.Parent = ExportWrap; ExportBox.BackgroundColor3 = TH.cd
            ExportBox.BorderSizePixel = 0; ExportBox.Size = UDim2.new(1,-74,1,0)
            ExportBox.LayoutOrder = 1; ExportBox.Font = Enum.Font.GothamBold
            ExportBox.Text = encodeTheme(editTheme); ExportBox.TextColor3 = TH.ac
            ExportBox.TextSize = 9; ExportBox.PlaceholderText = "โค้ดธีม"
            ExportBox.ClearTextOnFocus = false; ExportBox.TextEditable = false
            local _exbc = Instance.new("UICorner"); _exbc.CornerRadius = UDim.new(0,6); _exbc.Parent = ExportBox
            local ExportStroke = Instance.new("UIStroke"); ExportStroke.Parent=ExportBox; ExportStroke.Color=TH.br; ExportStroke.Thickness=1
            onThemeChange(function(t) tw(ExportBox,{BackgroundColor3=t.cd,TextColor3=t.ac},0.22); ExportStroke.Color=t.br end)

            local CopyBtn = Instance.new("TextButton")
            CopyBtn.Parent = ExportWrap; CopyBtn.BackgroundColor3 = TH.dk
            CopyBtn.BorderSizePixel = 0; CopyBtn.Size = UDim2.new(0,68,1,0)
            CopyBtn.LayoutOrder = 2; CopyBtn.AutoButtonColor = false
            CopyBtn.Font = Enum.Font.GothamBold; CopyBtn.Text = "📋 Copy"
            CopyBtn.TextColor3 = TH.ac; CopyBtn.TextSize = 10
            local _cpbc = Instance.new("UICorner"); _cpbc.CornerRadius = UDim.new(0,6); _cpbc.Parent = CopyBtn

            CopyBtn.MouseButton1Down:Connect(function() tw(CopyBtn,{Size=UDim2.new(0,64,1,0)},0.08,Enum.EasingStyle.Sine) end)
            CopyBtn.MouseButton1Click:Connect(function()
                tw(CopyBtn,{Size=UDim2.new(0,68,1,0)},0.12,Enum.EasingStyle.Back)
                ExportBox.Text = encodeTheme(editTheme)
                -- copy to clipboard (exploit env)
                local ok = pcall(function()
                    if setclipboard then
                        setclipboard(ExportBox.Text)
                    elseif Clipboard and Clipboard.set then
                        Clipboard.set(ExportBox.Text)
                    end
                end)
                -- fallback: select all text ให้ user copy เอง
                ExportBox.TextEditable = true
                ExportBox:CaptureFocus()
                ExportBox.TextEditable = false
                -- feedback
                CopyBtn.Text = ok and "✓ Copied" or "✓ Select"
                tw(CopyBtn,{BackgroundColor3=TH.ac,TextColor3=Color3.new(1,1,1)},0.1)
                task.delay(1.5, function()
                    CopyBtn.Text = "📋 Copy"
                    tw(CopyBtn,{BackgroundColor3=TH.dk,TextColor3=TH.ac},0.2)
                end)
            end)
            CopyBtn.MouseEnter:Connect(function() tw(CopyBtn,{BackgroundColor3=TH.ac,TextColor3=Color3.new(1,1,1)},0.14) end)
            CopyBtn.MouseLeave:Connect(function() tw(CopyBtn,{BackgroundColor3=TH.dk,TextColor3=TH.ac},0.14) end)
            onThemeChange(function(t) tw(CopyBtn,{BackgroundColor3=t.dk,TextColor3=t.ac},0.22) end)

            -- ── Import row ──────────────────────────────────────
            local ImportLbl = Instance.new("TextLabel")
            ImportLbl.Parent = Page; ImportLbl.BackgroundTransparency = 1
            ImportLbl.Size = UDim2.new(1,-28,0,14); ImportLbl.Font = Enum.Font.Gotham
            ImportLbl.Text = "วางโค้ดจากเพื่อน — กด โหลด เพื่อใช้"
            ImportLbl.TextColor3 = TH.su; ImportLbl.TextSize = 9
            ImportLbl.TextXAlignment = Enum.TextXAlignment.Left
            onThemeChange(function(t) tw(ImportLbl,{TextColor3=t.su},0.22) end)

            local ImportWrap = Instance.new("Frame")
            ImportWrap.Parent = Page; ImportWrap.BackgroundTransparency = 1
            ImportWrap.BorderSizePixel = 0; ImportWrap.Size = UDim2.new(1,-28,0,30)
            local ImportLayout = Instance.new("UIListLayout")
            ImportLayout.Parent = ImportWrap; ImportLayout.FillDirection = Enum.FillDirection.Horizontal
            ImportLayout.SortOrder = Enum.SortOrder.LayoutOrder; ImportLayout.Padding = UDim.new(0,6)
            ImportLayout.VerticalAlignment = Enum.VerticalAlignment.Center

            local ImportBox = Instance.new("TextBox")
            ImportBox.Parent = ImportWrap; ImportBox.BackgroundColor3 = TH.cd
            ImportBox.BorderSizePixel = 0; ImportBox.Size = UDim2.new(1,-74,1,0)
            ImportBox.LayoutOrder = 1; ImportBox.Font = Enum.Font.GothamBold
            ImportBox.Text = ""; ImportBox.TextColor3 = WT
            ImportBox.TextSize = 9; ImportBox.PlaceholderText = "วางโค้ดที่นี่..."
            ImportBox.ClearTextOnFocus = false
            local _imbc = Instance.new("UICorner"); _imbc.CornerRadius = UDim.new(0,6); _imbc.Parent = ImportBox
            local ImportStroke = Instance.new("UIStroke"); ImportStroke.Parent=ImportBox; ImportStroke.Color=TH.br; ImportStroke.Thickness=1
            onThemeChange(function(t) tw(ImportBox,{BackgroundColor3=t.cd},0.22); ImportStroke.Color=t.br end)

            local LoadBtn = Instance.new("TextButton")
            LoadBtn.Parent = ImportWrap; LoadBtn.BackgroundColor3 = TH.dk
            LoadBtn.BorderSizePixel = 0; LoadBtn.Size = UDim2.new(0,68,1,0)
            LoadBtn.LayoutOrder = 2; LoadBtn.AutoButtonColor = false
            LoadBtn.Font = Enum.Font.GothamBold; LoadBtn.Text = "โหลด"
            LoadBtn.TextColor3 = TH.ac; LoadBtn.TextSize = 11
            local _ldbc = Instance.new("UICorner"); _ldbc.CornerRadius = UDim.new(0,6); _ldbc.Parent = LoadBtn

            local function flashImport(ok)
                tw(ImportStroke, {Color = ok and TH.ac or Color3.fromRGB(240,60,60)}, 0.1)
                task.delay(1, function() ImportStroke.Color = TH.br end)
            end

            local function applyDecodedTheme(decoded)
                for k, v in pairs(decoded) do editTheme[k] = v end
                -- อัปเดต row swatches + chips + picker
                for i, def in ipairs(colorDefs) do
                    local nc = editTheme[def.key]
                    if rowEls[i] then
                        tw(rowEls[i].swatch, {BackgroundColor3=nc}, 0.15)
                        rowEls[i].hexLbl.Text = "#"..color3ToHex(nc)
                    end
                    if previewChips[i] then
                        tw(previewChips[i].chip, {BackgroundColor3=nc}, 0.15)
                    end
                end
                ExportBox.Text = encodeTheme(editTheme)
                -- reset picker ถ้าเปิดอยู่
                if selIdx >= 1 then
                    local key = colorDefs[selIdx].key
                    ph, ps, pv = color3ToHsv(editTheme[key])
                    syncPicker()
            ...
            LoadBtn.MouseButton1Down:Connect(function() tw(LoadBtn,{Size=UDim2.new(0,64,1,0)},0.08,Enum.EasingStyle.Sine) end)
            LoadBtn.MouseButton1Click:Connect(function()
                tw(LoadBtn,{Size=UDim2.new(0,68,1,0)},0.12,Enum.EasingStyle.Back)
                local code = ImportBox.Text
                local importName = NameBox.Text ~= "" and NameBox.Text or nil
                local decoded = decodeTheme(code, importName)
                if decoded then
                    applyDecodedTheme(decoded)
                    -- ถ้ามีชื่อใน code ให้เติมใน NameBox
                    if decoded.name and decoded.name ~= "Imported" and NameBox.Text == "" then
                        NameBox.Text = decoded.name
                    end
                    flashImport(true)
                    ImportBox.Text = ""
                    LoadBtn.Text = "✓ โหลดแล้ว"
                    tw(LoadBtn,{BackgroundColor3=TH.ac,TextColor3=Color3.new(1,1,1)},0.1)
                    task.delay(1.5, function()
                        LoadBtn.Text = "โหลด"
                        tw(LoadBtn,{BackgroundColor3=TH.dk,TextColor3=TH.ac},0.2)
                    end)
                else
                    flashImport(false)
                    LoadBtn.Text = "✗ โค้ดผิด"
                    tw(LoadBtn,{BackgroundColor3=Color3.fromRGB(200,50,50),TextColor3=Color3.new(1,1,1)},0.1)
                    task.delay(1.5, function()
                        LoadBtn.Text = "โหลด"
                        tw(LoadBtn,{BackgroundColor3=TH.dk,TextColor3=TH.ac},0.2)
                    end)
            ...
            LoadBtn.MouseEnter:Connect(function() tw(LoadBtn,{BackgroundColor3=TH.ac,TextColor3=Color3.new(1,1,1)},0.14) end)
            LoadBtn.MouseLeave:Connect(function() tw(LoadBtn,{BackgroundColor3=TH.dk,TextColor3=TH.ac},0.14) end)
            onThemeChange(function(t) tw(LoadBtn,{BackgroundColor3=t.dk,TextColor3=t.ac},0.22) end)

            -- sync ExportBox เมื่อแก้สีใน picker
            local origSync = syncPicker
            syncPicker = function()
                origSync()
                ExportBox.Text = encodeTheme(editTheme)
            end
